...
| 客户端 | PTY 设备 | 用途 |
| --- | --- | --- |
| `atcmd` | `/dev/ttyOUT2` | FM150 Web AT 控制页默认使用的通道 |
| `atcmd11` | `/dev/ttyOUT` | 备用 AT 通道 |
...
原项目的 systemd 单元默认将 `/dev/smd7` 映射到 `ttyOUT2`、将 `/dev/smd11` 映射到 `ttyOUT`。不同 FM150 固件的底层节点可能不同，因此部署脚本不会自动启用或覆盖这些单元；只有在确认节点与映射一致后，才手工启用对应服务。
